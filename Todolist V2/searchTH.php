...
$query_todolist = sprintf("SELECT * FROM todolist WHERE name LIKE %s or todolist.`date` LIKE %s", GetSQLValueString("%" . $colname_todolist . "%", "text"),GetSQLValueString("%" . $colname_todolist . "%", "text"));
...
<?php if($_SESSION['MM_Username']==$row_user['UserName']){?>
  ...
  <table class="table">
	...
	<?php }else{ ?><tr>
      ...
      <td><textarea cols="25" rows="2" disabled><?php echo $row_todolist['decription']; ?></textarea></td>
      <td>เสร็จสมบูรณ์</td>
      <td><a href="deleteTH.php?id=<?php echo $row_todolist['idToDoList']; ?>" onClick="remove()">ลบ</a></td>
